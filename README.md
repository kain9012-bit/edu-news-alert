...
세 파일은 비공개 GitHub Actions 결과 파일 `daily-education-trend-report`로 묶어 14일간만 보관합니다. Gmail은 사용하지 않습니다. Windows 자동 수신기가 GitHub의 결과 목록과 JSON 검증 상태를 확인한 뒤 지정 폴더에 내려받으며, 보관 기한이 지나면 GitHub에서 자동으로 삭제됩니다.
## Gemini API 설정
