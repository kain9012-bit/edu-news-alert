...
교육부와 16개 시도교육청 본청 보도자료를 평일 오전 6시 5분에 수집하고, Gemini가 교육행정 담당자에게 의미 있는 교육동향을 선별·분류하는 AI Agent 하네스입니다. 최근 자료는 Chrome·Edge 사이드바에서 확인할 수 있으며, 선별 결과는 별도의 비공개 하네스에서 요약·분석·전북교육 적용 검토를 거쳐 HTML·HWPX 내부 보고서로 생성합니다. 전북교육청 본청 자료는 보고서 마지막에 별도 구역으로 모아 내용 요약과 원문 링크만 제공합니다.
...
flowchart LR
    ...
    F --> I[교육지원청 자료 사전 제외]
    I --> J[타 시도 사실정리 Agent]
    J --> K[타 시도 동향분석·적용검토 Agent]
    K --> L[근거 검증 Agent]
    P --> Q[전북 보도자료 전용 요약 Agent]
    Q --> L
    L -->|REVISE| R[문제 문장 자동 재작성]
    R --> L
    L -->|PASS| M[기준 JSON: 전국 동향·전북 요약]
    M --> N[HTML·HWPX]
...
4. **사실정리 Agent**: 선정된 타 시도 자료에서 내용 요약 1~5개와 근거 사실을 추출합니다. 원문에 없는 정보나 긴 복사는 허용하지 않습니다.
5. **동향분석 Agent**: 타 시도 자료의 검증된 사실만 받아 교육동향 분석 1~5개와 전북교육 적용 검토 0~5개를 작성합니다. 전북교육청 자료는 이 단계에서 제외하며, 부서를 지정하거나 확정된 업무 지시처럼 표현하지 않습니다.
6. **전북 보도자료 요약 Agent**: AI의 교육동향 채택 여부나 중요도와 관계없이 분석 기간에 수집된 전북교육청 본청 보도자료 전체를 각각 1~2개 항목으로 짧게 요약합니다. 분석과 적용 검토는 작성하지 않습니다.
7. **내부 보고서 검증·수정 Agent**: 원문과 요약·분석·적용 검토를 대조해 근거 없는 숫자·기관·성과, 반복, 과장, 부서 지정, 장문 복사를 검사합니다. 문제가 있으면 해당 항목을 원문과 검증 사유에 맞춰 최대 두 차례 자동 재작성하고 다시 검증합니다. 반복 수정에도 해결되지 않은 예외는 실패 문구 없이 원문 요약과 링크만 배포본에 남기며, 상세 사유는 기준 JSON에만 기록합니다. 전북교육청 자료는 요약만 검증합니다.

Gemini 응답은 단계별 JSON 계약으로 검사합니다. 계약을 어기면 재시도하고 계속 실패하면 규칙 기반 대체 판정을 사용합니다. 보고서 수정 횟수와 최종 요약 전용 처리 건수, 검증 결과, 단계별 실행 시간과 Gemini 토큰 사용량은 최종 결과에 기록됩니다.
...
HTML 인쇄 시 첫 페이지는 표지·요약·목차에서 끝나며, 전국 교육동향은 다음 페이지부터 보도자료별 한 페이지 단위로 시작합니다. 목차 항목이 많으면 인쇄할 때만 글자 크기·줄간격·항목 여백을 단계적으로 줄여 첫 페이지 안에 맞춥니다. 문서 마지막의 전북교육청 보도자료는 항목이 중간에서 나뉘지 않도록 한 페이지에 최대 두 건씩 배치합니다.
HTML·HWPX의 전국 교육동향 목차에는 각 제목 앞에 `(교육부)`, `(대전)`과 같은 기관 지역명을 표시합니다. 전북교육청 보도자료는 별도 구역에 모이므로 제목 앞에 지역명을 반복하지 않습니다. 넓은 화면에서 HTML을 열면 왼쪽에 같은 순서의 빠른 목차가 고정되어 각 보도자료와 첫 페이지 목차로 바로 이동할 수 있으며, 이 컨트롤러는 인쇄물에는 나타나지 않습니다.
...
- 전남광주통합특별시교육청: 본청 전용 RSS 피드([`/rss/S1N1.xml`](https://www.jngjedu.kr/rss/S1N1.xml))로 수집. 통합청 사이트는 목록 페이지의 섹션 필터와 페이지네이션이 HTTP 요청에서 동작하지 않아 본청 자료가 1페이지 밖으로 밀리면 누락되므로, 서버가 본청만 담아 주는 RSS를 사용해 직속기관·교육지원청·학교 자료를 원천적으로 제외
