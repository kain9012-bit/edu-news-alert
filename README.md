...
6. **내부 보고서 검증 Agent**: 원문과 요약·분석·적용 검토를 대조해 근거 없는 숫자·기관·성과, 반복, 과장, 부서 지정, 장문 복사를 검사합니다. 통과하지 못한 항목은 배포본에서 제외합니다. 다만 중요도가 높은 항목(기본 4점 이상)은 검증에서 걸려도 통째로 버리지 않고, AI 분석·적용 검토만 생략한 채 원문 요약과 링크를 남겨 「검토 필요」로 표시한다. 중요한 정책 자료가 조용히 사라지지 않도록 하기 위함이다.
