...
내부 보고서는 `public`에 저장하거나 공개 저장소에 커밋하지 않습니다. 비공개 저장소 [`kain9012-bit/edu-news-alert-private-reports`](https://github.com/kain9012-bit/edu-news-alert-private-reports)가 최신 공개 선별 결과를 받아 다음 세 파일을 동일한 기준 JSON에서 생성하고, 14일 보관되는 비공개 Actions Artifact로 제공합니다.

- `오늘의 교육동향 (YYYYMMDD).json`: 검증된 기준 데이터와 모델·토큰·예상 비용 기록
- `오늘의 교육동향 (YYYYMMDD).html`: 기본 배포본
- `오늘의 교육동향 (YYYYMMDD).hwpx`: 한글 문서 배포본

비공개 저장소의 `Actions`에서 최신 실행을 열고 화면 아래 `Artifacts`의 `오늘의-교육동향-YYYYMMDD`를 누르면 ZIP으로 받을 수 있습니다.
